...
# GMT Example 48
#
# Illustrate trimmed lines with vectors, networks,
# map embellishments, and curved titles
...
while read lon lat az1 az2 label just off; do
	echo $lon $lat $label | \
		gmt text -DJ${off}+v0.5p,white -F+f16p+j${just} -N
	echo $lon $lat $label | \
		gmt text -DJ${off}+v0.25p -F+f16p+j${just} -N -Gwhite -W0.25p -C+tO
done < airports.txt
...
# Make an arc of radius 11.5 cm from 45 to 135 degrees around map center
# and use it to place text
gmt math -T45/135/1 T -C0 COSD -C1 SIND -Ca 11.5 MUL = path.txt
# Move up 9 cm so its origin is at the map center
gmt plot -R-9/9/0/18 -Jx1c -Y9c path.txt \
	-Sqn1:+l"IMPORTANT PACIFIC AIRPORTS"+v+f32p -Wfaint,white -N
