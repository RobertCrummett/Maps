...
# GMT Example 18
#
# Isolate all seamounts the exceed 50 mGals in amplitude
# and are within 200 km of the Pratt seamount. First put
# 50 mGal contours on disk. Then get mean location of
# each closed polygon. Then select points within 200 km
# of the Pratt seamount.
#
...
# Now determine centers of each enclosed seamount > 50 mGal
# but only plot the ones within 200 km of the Pratt seamount.
...
gmt select -Cpratt.txt+d200k centers.txt -fg | \
    gmt plot -Sc0.1c -Gred -Wthinnest
gmt plot -ST0.25c -Gyellow -Wthinnest pratt.txt
# Then report the volume and area of these seamounts only
# by masking out the data outside the 200 km radius circle
