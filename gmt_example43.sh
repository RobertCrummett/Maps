...
gmt regress -Ey -Nw -i0:1+l $file -Fxm -T-2/6/2+n > ls_line.txt
grep -v '^>' model.txt > A.txt
grep -v '^#' $file > B.txt
awk '{if ($7 == 0) printf "%dp\n", NR}' A.txt > sed.txt
gmt makecpt -Clightred,green -T0/2/1 -F+c -N
gmt basemap -R0.01/1e6/0.1/1e5 -JX15cl -Ba1pf3 -Bx+l"Log@-10@- body weight
	(kg)" -By+l"Log@-10@- brain weight (g)" -BWSne+glightblue -Y10c
gmt plot -R-2/6/-1/5 -JX15c rls_line.txt -L+yt -Glightgoldenrod
sed -n -f sed.txt B.txt | \
	gmt text -R0.01/1e6/0.1/1e5 -JX15cl -F+f12p+jRM -Dj10p
...
EOF
awk '{print NR, $6, $7}' A.txt | \
	gmt plot -Sb1q+b0 -W0.25p -C
gmt basemap -Bafg100 -Bx+l"Animal index number" -By+l"z-score" -BWSne
rm -f *.txt
gmt end show
