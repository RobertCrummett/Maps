...
# Demonstrate how grdfft can be used to compute spectral coherence
...
gmt grdimage ${G}_tmp.nc -I+a0+nt1 -R${G}_tmp.nc -Jx${scl}i -X3.25i
gmt plot bbox -L -W0.5p,-
gmt basemap -R-100/91/-94/97 -Jx${sclkm}i -Ba -BWSne+t"Detrended and extended"

gmt set FONT_TITLE 24p
gmt plot -R2/160/0/1 -JX-6il/2.5i -Bxa2f3g3+u" km" -Byafg0.5+l"Coherency@+2@+" \
    -BWsNe+t"Coherency between gravity and bathymetry" -X-3.25i -Y3.3i \
    cross.txt -i0,15 -W0.5p
